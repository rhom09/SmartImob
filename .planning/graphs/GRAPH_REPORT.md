...
1. `Button` - 24 edges
2. `Card` - 21 edges
3. `CardContent` - 21 edges
4. `formatCurrency()` - 17 edges
5. `CardHeader` - 16 edges
6. `CardTitle` - 14 edges
7. `Input` - 14 edges
...
- `NovoReciboPage()` --calls--> `formatCurrency()`  [EXTRACTED]
  frontend/src/app/financeiro/recibos/novo/page.tsx → frontend/src/lib/utils.ts
...
Cohesion: 0.09
Nodes (42): ContractForm(), ContractFormData, ContractFormProps, contractSchema, TIPOS_DESPESA, FinanceiroPage(), FinancialSummary, DetalhesClientePage() (+34 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (30): globalForPrisma, authMiddleware(), AuthRequest, data, parsed, router, where, parsed (+22 more)
...
## Knowledge Gaps
- **114 isolated node(s):** `prisma`, `app`, `globalForPrisma`, `AuthRequest`, `router` (+109 more)
